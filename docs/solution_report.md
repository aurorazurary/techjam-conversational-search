...
The union is reranked using category coverage, constraint token coverage, exact
normalized phrase matches, title overlap, price compatibility, profile-tag overlap,
and small rating/popularity tie-breakers. Failed recommendations are not repeated,
which provides deterministic candidate diversification over multiple turns.
...
The final public evaluation completed in 116.94 seconds on the development machine.
That run covered 200 sessions and 425 calls to `respond`, or about 0.58 seconds per
session and 0.28 seconds per response end to end, including catalog loading and index
...
| MRR | 0.068034 | 0.626956 |
| MTTC | 9.81 | 2.13 |
| Efficiency | 0.119 | 0.887 |
| TechnicalScore | 0.106710 | 0.862987 |

Scenario Hit Rate@10 is 1.0 for Browsing, Boundary, and Intent Override, and
0.9875 for Buying. These are development-set results and are not a guarantee of
